...
class MarketScanner:
    ...
    # MarketScanner.py 내 추가 제안 메서드

    def get_volume_spike_list(self, market='0', volume_filter='1'):
        """
        [CpSvr7043] 거래량 급증 종목 조회 (vol_ratio 확보용)
        market: '0'전체, '1'거래소, '2'코스닥
        volume_filter: '1'1만주, '2'5만주, '3'10만주 이상
        """
        obj = win32com.client.Dispatch("CpSysDib.CpSvr7043")
        obj.SetInputValue(0, market)        # 시장 구분
        obj.SetInputValue(1, '2')           # '1'급증, '2'거래량 갱신 (RVOL 포착에 유리)
        obj.SetInputValue(2, volume_filter) # 거래량 조건
        obj.SetInputValue(3, '1')           # 가격 조건 (1% 이상 상승)
        obj.SetInputValue(4, '0')           # 관리종목 제외
        ...
        for i in range(count):
            ...
            results[code] = {
                'vol_ratio': obj.GetDataValue(3, i), # 🎯 전일 대비 거래량 비율 (%)
                'current': obj.GetDataValue(4, i)    # 현재가
            }
        return results

    def get_market_status(self):
        """KOSPI(001), KOSDAQ(101) 지수 등락 확인"""
        obj = win32com.client.Dispatch("CpSysDib.CpSvr7089")
        # 지수 관련 코드를 넣어 실시간 지수 방향성을 반환하는 로직 추가 가능
        pass
